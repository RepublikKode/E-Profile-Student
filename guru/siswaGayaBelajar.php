...
$id = $_GET['id'];
$getDetailSiswa = query("SELECT * FROM hasil_gaya_belajar WHERE id = '$id'")[0];
...
<div class="container" id="top" style="width: 100%; height: max-content;">
    <div class="card my-5 shadow-md border-success container d-flex flex-column" style="width: 100%; height: max-content;">
        <div class="card my-3 overflow-hidden d-flex flex-row align-items-center shadow-md">
            ...
            <div class="border p-1 flex-fill" style="width: 100%;">
                <p class="h5 d-flex mt-3"><?= $getDetailSiswa['user']; ?>-<span class="fw-bold"> <?= $getDetailSiswa['gaya_belajar']; ?></span></p>
                <hr>
                ...
                <div class="d-flex flex-column gap-3">
                    <?php foreach ($getSoal as $index => $gs) : ?>
                        ...
                        <ul class="d-flex flex-column gap-3">
                            <li class="bg-primary text-white p-2 rounded"><?= $gs['opsi_satu'] ?></li>
                            <li class="bg-primary text-white p-2 rounded"><?= $gs['opsi_dua'] ?></li>
                            <li class="bg-primary text-white p-2 rounded"><?= $gs['opsi_tiga'] ?></li>
                        </ul>
                        <div class="bg-success text-white p-2 rounded">
                            Jawaban : <?= $getDetailSiswa['soal_' . ($index + 1)]; ?>
                        </div>
                        <hr>
                    <?php endforeach; ?>
